...
export function MemberModal({ member, onSave, onClose }) {
  ...
  return (
    <Modal title={member ? "Edit Member" : "Add Member"} onClose={onClose} footer={<><div onClick={onClose} style={BTN("#334155","#cbd5e1")}>Cancel</div><div onClick={() => { if(f.firstName && f.lastName) onSave({ ...f, id: member?.id || Date.now().toString() }) }} style={BTN("linear-gradient(135deg,#3b82f6,#6366f1)")}>{member ? "Save Changes" : "Add Member"}</div></>}>
  ...
  return (
    <Modal title={m.firstName + " " + m.lastName} onClose={onClose} footer={<div onClick={onClose} style={{...BTN("#334155","#cbd5e1"),width:"100%",textAlign:"center"}}>Close</div>}>
      <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
        ...
        {m.email && <div><p style={{color:"#64748b",fontSize:"12px",margin:"0 0 2px"}}>Email</p><p style={{color:"#f1f5f9",fontSize:"14px",margin:0}}>{m.email}</p></div>}
