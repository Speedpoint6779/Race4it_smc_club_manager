    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <p style={{ color: "#64748b", fontSize: "13px", margin: 0 }}>Weekly meeting speaker schedule — managed via Google Sheet</p>
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <a href={SHEET_URL} target="_blank" rel="noopener noreferrer" style={{ ...BTN("linear-gradient(135deg,#3b82f6,#6366f1)"), display: "flex", alignItems: "center", gap: "6px", textDecoration: "none" }}>
      ...
      <div style={{ background: "#1e293b", borderRadius: "12px", border: "1px solid #334155", overflow: "hidden" }}>
        ...
        {fsp.map((s, i) => {
          ...
          if (s.noMeeting) return (
            <div key={s.id} style={{ display: "grid", gridTemplateColumns: "90px 1fr", borderBottom: i < fsp.length - 1 ? "1px solid #334155" : "none", background: "#7f1d1d10", opacity: past ? 0.6 : 1, alignItems: "center" }}>
              <div style={{ padding: "12px 14px" }}><span style={{ color: "#f1f5f9", fontSize: "14px", fontWeight: "500" }}>{fmtDate(s.date)}</span></div>
              <div style={{ padding: "12px 14px", color: "#f87171", fontSize: "13px", fontWeight: "600", fontStyle: "italic" }}>NO MEETING — {s.reason}</div>
            </div>
          );
          return (
      ...
      <div style={{ marginTop: "12px", color: "#64748b", fontSize: "12px" }}>
        To add or edit speakers, update the <a href={SHEET_URL} target="_blank" rel="noopener noreferrer" style={{ color: "#3b82f6" }}>Google Sheet</a> then click Sync from Sheet.
      </div>
    </div>
  );
}
