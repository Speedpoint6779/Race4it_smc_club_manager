...
export function SpeakersPage({ speakers, setSpeakers, flash, syncing, setSyncing, setEditS, setShowSM, setConfDelS }) {
  const [spFil, setSpFil] = window.React.useState("upcoming");
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <div onClick={syncFromSheet} style={{ ...BTN("#334155", "#cbd5e1"), display: "flex", alignItems: "center", gap: "6px", opacity: syncing ? 0.5 : 1, pointerEvents: syncing ? "none" : "auto" }}>
            <Icons.Refresh />{syncing ? "Syncing..." : "Sync from Sheet"}
          </div>
          <a href={SHEET_URL} target="_blank" rel="noopener noreferrer" style={{ ...BTN("#334155", "#cbd5e1"), display: "flex", alignItems: "center", gap: "6px", textDecoration: "none" }}>
            <Icons.Ext />Google Sheet
          </a>
          <div onClick={() => { setEditS(null); setShowSM(true); }} style={{ ...BTN("linear-gradient(135deg,#3b82f6,#6366f1)"), display: "flex", alignItems: "center", gap: "6px" }}>
            <Icons.Plus />Add Speaker
          </div>
        </div>
      </div>
            ...
            <div key={s.id} style={{ display: "grid", gridTemplateColumns: "90px 1fr 70px", borderBottom: i < fsp.length - 1 ? "1px solid #334155" : "none", background: "#7f1d1d10", opacity: past ? 0.6 : 1, alignItems: "center" }}>
              ...
              <div style={{ padding: "12px 14px" }}>
                <div style={{ display: "flex", gap: "4px" }}>
                  <div onClick={() => { setEditS(s); setShowSM(true); }} style={{ padding: "5px 8px", background: "#334155", borderRadius: "6px", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center" }}><Icons.Edit /></div>
                  <div onClick={() => setConfDelS(s)} style={{ padding: "5px 8px", background: "#334155", borderRadius: "6px", color: "#f87171", cursor: "pointer", display: "flex", alignItems: "center" }}><Icons.Trash /></div>
                </div>
              </div>
            </div>
          );
            ...
            <div key={s.id} style={{ display: "grid", gridTemplateColumns: sCols, borderBottom: i < fsp.length - 1 ? "1px solid #334155" : "none", background: isNext ? "#3b82f610" : "transparent", opacity: past ? 0.6 : 1, alignItems: "center" }}>
              <div style={{ padding: "12px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ color: isNext ? "#93c5fd" : "#f1f5f9", fontSize: "14px", fontWeight: isNext ? "700" : "500" }}>{fmtDate(s.date)}</span>
                  {isNext && <span style={{ fontSize: "10px", padding: "2px 6px", borderRadius: "99px", background: "#3b82f630", color: "#93c5fd", fontWeight: "600" }}>NEXT</span>}
                </div>
              </div>
