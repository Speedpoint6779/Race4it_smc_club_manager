...
function SentBodyPreview({ html }) {
  ...
  const plainText = html
    ...
    .trim();
  return (
    <div style={{ color: "var(--text-primary)", fontSize: "15px", lineHeight: "1.7", whiteSpace: "pre-wrap", fontFamily: "inherit" }}>
      {plainText}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        <h1 style={{ color: "var(--text-heading)", fontSize: "26px", fontWeight: "700", margin: 0 }}>Email</h1>
        <div onClick={() => { setEmailPre([]); setShowEM(true); }} style={{ ...BTN("var(--accent-gradient)"), display: "flex", alignItems: "center", gap: "6px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "24px" }}>
        <div onClick={() => { setEmailPre(members.filter(m => m.status === "active").map(m => m.id)); setShowEM(true); }} style={{ padding: "20px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", cursor: "pointer" }}>
          <div style={{ fontSize: "20px", marginBottom: "6px" }}>📣</div>
          <div style={{ color: "var(--text-heading)", fontSize: "16px", fontWeight: "600", marginBottom: "4px" }}>Email All Active</div>
          <div style={{ color: "var(--text-muted)", fontSize: "14px" }}>{ac} recipients</div>
        </div>
        <div onClick={() => { setEmailPre(mwd.filter(m => m._ds !== "paid" && m.status === "active").map(m => m.id)); setShowEM(true); }} style={{ padding: "20px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", cursor: "pointer" }}>
          <div style={{ fontSize: "20px", marginBottom: "6px" }}>💰</div>
          <div style={{ color: "var(--text-heading)", fontSize: "16px", fontWeight: "600", marginBottom: "4px" }}>Email Unpaid Members</div>
          <div style={{ color: "var(--text-muted)", fontSize: "14px" }}>{mwd.filter(m => m._ds !== "paid" && m.status === "active").length} recipients</div>
        </div>
        <div onClick={() => setShowListManager(true)} style={{ padding: "20px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", cursor: "pointer" }}>
          <div style={{ fontSize: "20px", marginBottom: "6px" }}>👥</div>
          <div style={{ color: "var(--text-heading)", fontSize: "16px", fontWeight: "600", marginBottom: "4px" }}>Saved Lists</div>
          <div style={{ color: "var(--text-muted)", fontSize: "14px" }}>{lists.length} list{lists.length !== 1 ? "s" : ""} saved</div>
        </div>
      </div>

      {/* Saved Lists Manager */}
      {showListManager && (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", marginBottom: "20px", overflow: "hidden" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: "1px solid var(--border)", background: "var(--bg-hover)" }}>
            <div style={{ color: "var(--text-heading)", fontSize: "16px", fontWeight: "600" }}>Saved Lists</div>
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <div onClick={() => { setEmailPre([]); setShowEM(true); setShowListManager(false); }} style={{ ...BTN("var(--accent-gradient)"), fontSize: "12px", padding: "6px 12px" }}>+ Create New List</div>
              <div onClick={() => setShowListManager(false)} style={{ color: "var(--text-muted)", cursor: "pointer", fontSize: "18px", padding: "4px 6px" }}>✕</div>
            </div>
          </div>
          {lists.length === 0 ? (
            <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>No saved lists yet. Compose an email and save your recipient selection as a list.</div>
          ) : (
            <div>
              {lists.map((list, i) => (
                <div key={list.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 20px", borderBottom: i < lists.length - 1 ? "1px solid var(--border)" : "none" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: "var(--text-heading)", fontSize: "15px", fontWeight: "600", marginBottom: "2px" }}>{list.name}</div>
                    <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>{list.member_ids.length} member{list.member_ids.length !== 1 ? "s" : ""}</div>
                  </div>
                  <div onClick={() => { setEmailPre(list.member_ids); setShowEM(true); setShowListManager(false); }} style={{ ...BTN("var(--restore-bg)"), fontSize: "13px", padding: "6px 14px", color: "var(--accent-text)", border: "1px solid var(--accent-border)", whiteSpace: "nowrap" }}>Email This List</div>
      ...
      {/* Warning */}
      {noEmail > 0 && (
        <div style={{ background: "var(--warning-bg)", border: "1px solid var(--warning-border)", borderRadius: "10px", padding: "12px 16px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "10px" }}>
          <Icons.Alert />
          <span style={{ color: "var(--warning-text)", fontSize: "14px" }}>{noEmail} active member{noEmail !== 1 ? "s have" : " has"} no email address on file — they won't receive emails.</span>
        </div>
      )}
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", marginBottom: "16px", overflow: "hidden" }}>
          ...
          <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border-light)", background: "var(--bg-hover)" }}>
            ...
            {openSent.recipient_emails && (
              <div style={{ color: "var(--text-muted)", fontSize: "13px", marginBottom: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{openSent.recipient_emails}</div>
            )}
      ...
      <div style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid var(--border)" }}>
          <div style={{ display: "flex", gap: "8px" }}>
            <div onClick={() => setTab("inbox")} style={{ ...tabStyle(tab === "inbox"), display: "flex", alignItems: "center", gap: "6px" }}>
              Inbox{unread > 0 && <span style={{ background: "var(--accent)", color: "white", borderRadius: "99px", fontSize: "10px", fontWeight: "700", padding: "1px 7px" }}>{unread}</span>}
            </div>
            ...
            <div onClick={() => setTab("trash")} style={{ ...tabStyle(tab === "trash"), display: "flex", alignItems: "center", gap: "6px" }}>
              🗑 Trash{trash.length > 0 && tab !== "trash" && <span style={{ background: "var(--text-muted)", color: "white", borderRadius: "99px", fontSize: "10px", fontWeight: "700", padding: "1px 7px" }}>{trash.length}</span>}
            </div>
          </div>
          <div onClick={refreshCurrent} style={{ color: "var(--text-muted)", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", fontSize: "13px" }}>
        ...
        {/* Sent Tab */}
        {tab === "sent" && (
          logLoading ? <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>Loading...</div>
          : log.length === 0 ? <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>No emails sent yet</div>
          : <div>{log.map((entry, i) => (
              <div key={entry.id} onClick={() => { setOpenMsg(null); setOpenSent(openSent?.id === entry.id ? null : entry); }}
                style={{ display: "flex", alignItems: "center", gap: "8px", padding: "14px 20px", borderBottom: i < log.length - 1 ? "1px solid var(--border)" : "none", background: openSent?.id === entry.id ? "var(--bg-selected)" : (i % 2 === 0 ? "var(--sent-stripe)" : "transparent"), cursor: "pointer", transition: "background 0.15s" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: "var(--text-muted)", fontSize: "13px", marginBottom: entry.recipient_emails ? "4px" : 0 }}>{fmtDate(entry.sent_at)}</div>
                  {entry.recipient_emails && <div style={{ color: "var(--text-faint)", fontSize: "13px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><span style={{ color: "var(--text-muted)" }}>To: </span>{entry.recipient_emails}</div>}
                </div>
        ...
        {/* Trash Tab */}
        {tab === "trash" && (
          trashLoading ? <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>Loading...</div>
          : trash.length === 0
            ? <div style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: "15px" }}>Trash is empty</div>
            : <>
                <div style={{ padding: "10px 20px", borderBottom: "1px solid var(--border)", background: "var(--bg-hover)", fontSize: "13px", color: "var(--text-muted)" }}>
                ...
                <div>{trash.map((item, i) => {
                  ...
                  return (
                    <div key={`${item._type}-${item.id}`} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "14px 20px", borderBottom: i < trash.length - 1 ? "1px solid var(--border)" : "none", background: i % 2 === 0 ? "var(--sent-stripe)" : "transparent" }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                          <span style={{ fontSize: "10px", padding: "2px 6px", borderRadius: "4px", background: isInbox ? "var(--trash-label-inbox-bg)" : "var(--trash-label-sent-bg)", color: isInbox ? "var(--trash-label-inbox-text)" : "var(--trash-label-sent-text)", fontWeight: "600", flexShrink: 0 }}>
                            {isInbox ? "INBOX" : "SENT"}
                          </span>
                          <div style={{ color: "var(--text-secondary)", fontSize: "15px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{subject}</div>
                        </div>
                        <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>{meta} · {fmtDate(date)}</div>
                      </div>
                      <div onClick={() => restoreItem(item)} title="Restore"
                        style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 10px", borderRadius: "6px", cursor: "pointer", background: "var(--restore-bg)", color: "var(--restore-text)", fontSize: "13px", fontWeight: "500", border: "1px solid var(--restore-border)", flexShrink: 0 }}>
                        ↩ Restore
                      </div>
...
function DelButton({ onClick, title }) {
  ...
  return (
    <div onClick={onClick} title={title} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "30px", height: "30px", borderRadius: "6px", cursor: "pointer", color: hover ? "#ef4444" : "var(--text-faint)", flexShrink: 0, background: hover ? "#ef444415" : "transparent", transition: "all 0.15s" }}>
